...
<div class="col-sm-8 col-md-8 col-lg-8 main">
                ...
                <div class="row likeWrapper justify-content-center">
                    <?php
                        $session = session();
                    ...
                    ?>
                     <?php 
                        ...
                        if(!empty($session->get('korisnik'))){
                            ...
                            $flag = true;
                                  $rezultati = $ocenioModel->where('idOcenjen',$drugiKorisnik->idKor)->findAll();
                                  foreach($rezultati as $res){
                                      if($res->idOcenio==$session->get('korisnik')->idKor && $res->tip_ocene == "pozitivna"){
                                      echo anchor("$controller/ocenaPozitivna/{$drugiKorisnik->idKor}", "Pozitivno",'class="btn btn-outline-danger btn-sm btn-danger"');
                                      $flag = false;
                                      }
                                  }
                            if($flag==true)echo  anchor("$controller/ocenaPozitivna/{$drugiKorisnik->idKor}", "Pozitivno",'class="btn btn-sm btn-light"');
                         } 
                      ?> 
                   &nbsp;&nbsp;&nbsp;
                      <?php 
                             $session = session();
                              if(!empty($session->get('korisnik'))){
                                  ...
                                  $rezultati = $ocenioModel->where('idOcenjen',$drugiKorisnik->idKor)->findAll();
                                  foreach($rezultati as $res){
                                      if($res->idOcenio==$session->get('korisnik')->idKor && $res->tip_ocene == "negativna"){
                                      echo anchor("$controller/ocenaNegativna/{$drugiKorisnik->idKor}", "Negativno",'class="btn btn-outline-danger btn-sm btn-danger"');
                                      $flag = false;
                                      }
                                  }
                                  if($flag == true)echo anchor("$controller/ocenaNegativna/{$drugiKorisnik->idKor}", "Negativno",'class="btn btn-sm btn-light"');
                               } 
                        ?> 
                    &nbsp;&nbsp;&nbsp;
